...
class VehicleSimulator:
    def __init__(self, shema_loic=False, shema_eliott=False):
        ...
        # soit la longueur du vehicule 70pixel = 4.5 m référentiel !!!
        # (4 petites voitures< X <5 grandes voitures)
        # Voie normale (écartement standard UIC de 1,435 m) -> 22.3 pixel
        self.rails_thickness = 22
        # largeur d’une autoroute est de 3,50 m pour les 2 × 2 voies -> 54.4 pixel
        self.road_L_thickness = self.road__R_thickness = 54/2

        if self.shema_eliott:
            # Draw rails
            self.init_rails(HEIGHT*2/5, self.rails_thickness)
            self.init_rails(HEIGHT*3/5, self.rails_thickness)

            # Draw roads
            self.init_roads(WIDTH/2-self.road_L_thickness, self.road_L_thickness)
            self.init_roads(WIDTH/2+self.road__R_thickness, self.road__R_thickness)

            # self.canvas.create_arc(HEIGHT*3/5+40, WIDTH/2+self.rails_thickness, HEIGHT+100, WIDTH/2+self.rails_thickness+platform_height, start=90, extent=90)
            # self.canvas.create_arc(*get_4points(HEIGHT/2, WIDTH/2, platform_width, platform_height), start=90, extent=90)
        ...
        # # droite modèles 30 et 60 degres
        # self.canvas.create_line(750, 0, 0, 375, width=4)
        # self.canvas.create_line(300, 0, 0, 600, width=4)

        # # Draw platform in good size (green)
        # 70pixel = 4.5 m (4 petites voitures< X <5 grandes voitures)
        # 156pixel = 10 m
        platform_width = 2*156 + self.road_L_thickness + self.road__R_thickness
        platform_height = 2*(156 - self.rails_thickness)
        self.canvas.create_polygon(*get_4points(HEIGHT/2, WIDTH/2, platform_width, platform_height), fill="", outline="green")

        # Draw vehicle
        if self.shema_loic:
            self.x = 248
        else:
            self.x = WIDTH/2+self.road__R_thickness
        self.y = HEIGHT-50
        ...
        # largeur 1.67 m -> 26
        # 70pixel = 4.5 m (4 petites voitures< X <5 grandes voitures)
        self.v_width, self.v_height = 26, 70
...
def run_episode(nb):
    ...
    for i in range(nb):
        ...
        # sim.shema_loic, sim.shema_eliott = False, True # differents modèle de shéma
